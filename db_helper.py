...
def get_order_status(order_id: int):
    print(f"Checking order status for order_id: {order_id}")  # Debug print

    # if order_id is None:  # Prevent query execution with None
    ...
    try:
        ...
        cursor.execute(query, (order_id,))  # Use parameterized query to prevent SQL injection

        result = cursor.fetchone()

        if result:
            return result["status"]  # ✅ Access 'status' field
        else:
